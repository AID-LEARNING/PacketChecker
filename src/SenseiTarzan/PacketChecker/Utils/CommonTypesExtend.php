...
final class CommonTypesExtend
{
	...
	public static function checkSkin(ByteBufferReader $in) : void
	{
		...
		for ($i = 0; $i < $personaPieceCount; ++$i) {
			...
			$isDefaultPiece = CommonTypes::getBool($in); //isDefaultPiece is true if the piece is from the marketplace, false if it's from the client (either custom or default skin)
			self::getString($in); //product id, can be empty string if not from marketplace
		}
